...
const handlerServer = (req, rep) => {
  ...
  const { url } = req;
  const passedUrl = URL.parse(url, true);
  const { pathname, query } = passedUrl;
  console.log(query.name);
  if (pathname === "/") {
    rep.end("Hello There");
  } else if (pathname === "/greetings") {
    rep.end(`hello ${query.name ? query.name : "there"} `);
  } else if (pathname === "/products") {
    rep.end(JSON.stringify(product));
  } else {
    rep.end("Something wrong!");
  }
};
